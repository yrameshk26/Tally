...
- [x] Run against the live API — totals and the per-registered-type split match
      what the brokerage's own app shows, within FX rounding.
- [x] Concentration is reported correctly against invested value. Invested can
      sit below net worth because managed portfolios report a balance with no
      position breakdown — not a bug, and `get_holdings by_account` shows both.
...
- [x] Moving an account to another profile survives a re-sync
- [ ] Seed each person's RRSP/TFSA limits into `room.json`
...
- [x] Merchant names arrive unnormalised — one company can land under several
      spellings and split across two rows in `top_merchants`. Fixed by merchant
      rules; the Merchants tab is where you merge them.
- [ ] A broken Item shows up in `plaid_status` but nothing surfaces it
      proactively — `login_required` and `INSTITUTION_NOT_RESPONDING` both sit
      there until someone looks. Consider a staleness banner or a sync-report
      warning when an Item has not synced in N days.
