...
/** Truncate to fit the label gutter; the full text stays in a <title>. */
function clipLabel(label: string, chars: number): string {
...
export function hBars(opts: {
  ...
  /**
   * viewBox width, which should roughly match the container's pixel width.
   * The SVG scales to fill its box, so a viewBox much narrower than the card
   * magnifies the text and a much wider one shrinks it to nothing. 360 suits a
   * half-width card; pass ~900 for a full-width one.
   */
  width?: number;
  empty?: string;
}): SafeHtml {
  const rows = opts.rows.filter((r) => r.value > 0);
  if (rows.length === 0) return html`<p class="empty">${opts.empty ?? 'Nothing to show yet.'}</p>`;
  const fmt = opts.format ?? money;
  const max = opts.max ?? Math.max(...rows.map((r) => r.value));
  const W = opts.width ?? 360;
  const valueW = Math.max(...rows.map((r) => fmt(r.value).length)) * 7 + 16;
  // Label column sized to the longest label, capped so a single long name
  // cannot squeeze the bars into nothing. Anything still over is truncated,
  // because an overflowing label escapes the card entirely.
  const longest = Math.max(...rows.map((r) => r.label.length));
  const labelW = Math.min(Math.max(longest * 6.4 + 12, 56), W * 0.34);
  const labelChars = Math.max(6, Math.floor((labelW - 12) / 6.4));
  ...
  const marks = rows
    .map((r, i) => {
      ...
      return (
        `<text class="label" x="${(labelW - 8).toFixed(1)}" y="${yy + barH / 2 + 4}" text-anchor="end">` +
        `${esc(clipLabel(r.label, labelChars))}<title>${esc(r.label)}</title></text>` +
