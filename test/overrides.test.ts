...
describe('categories', () => {
  it('normalises a name so the pickers stay consistent', async () => {
    const { normalizeCategory } = await import('../src/overrides.ts');
    expect(normalizeCategory('  child care ')).toBe('CHILD_CARE');
    expect(normalizeCategory('Pet / vet')).toBe('PET_VET');
    expect(normalizeCategory('Dining & drinks!')).toBe('DINING_DRINKS');
  });

  it('exists before anything is filed under it', async () => {
    const { addCategory } = await import('../src/overrides.ts');
    addCategory(db, 'Childcare');
    // The whole point of the table: a derived-from-usage list could not offer
    // this in a picker, so it could never be assigned in the first place.
    expect(knownCategories(db)).toContain('CHILDCARE');
    expect(groupByCategory(getTransactions(db, { limit: 50 })).map((c) => c.category)).not.toContain(
      'CHILDCARE',
    );
  });

  it('refuses a name that normalises to nothing, and is idempotent', async () => {
    const { addCategory, customCategories } = await import('../src/overrides.ts');
    expect(() => addCategory(db, '  !! ')).toThrow(/needs a name/);
    addCategory(db, 'Travel');
    addCategory(db, 'travel');
    expect(customCategories(db).filter((c) => c === 'TRAVEL')).toHaveLength(1);
  });

  it('removing one leaves transactions filed under it alone', async () => {
    const { addCategory, deleteCategory, setMerchantCategory } = await import('../src/overrides.ts');
    addCategory(db, 'Childcare');
    setMerchantCategory(db, 'Sdm', 'CHILDCARE');
    expect(byId('tx:3')?.category).toBe('CHILDCARE');
    expect(deleteCategory(db, 'CHILDCARE')).toBe(true);
    expect(byId('tx:3')?.category).toBe('CHILDCARE');
  });
});

describe('setMerchantCategory', () => {
  it('files every transaction from a merchant, past and future', async () => {
    const { setMerchantCategory } = await import('../src/overrides.ts');
    setMerchantCategory(db, 'Primmum Insurance Co', 'INSURANCE');
    expect(byId('tx:1')?.category).toBe('INSURANCE');
  });

  it('does not capture a merchant whose name merely starts the same way', async () => {
    const { setMerchantCategory } = await import('../src/overrides.ts');
    // "Primmum Insurance Comp" contains "Primmum Insurance Co". A contains rule
    // would swallow it — as it would file Amazon Web Services under whatever
    // was picked for Amazon.
    setMerchantCategory(db, 'Primmum Insurance Co', 'INSURANCE');
    expect(byId('tx:2')?.category).toBe('GENERAL_SERVICES');
    expect(byId('tx:2')?.merchant).toBe('Primmum Insurance Comp');
  });

  it('still merges deliberately, through a contains rule', () => {
    addMerchantRule(db, { pattern: 'primmum', merchant: 'Primmum Insurance' });
    expect(byId('tx:1')?.merchant).toBe('Primmum Insurance');
    expect(byId('tx:2')?.merchant).toBe('Primmum Insurance');
  });

  it('creates the category as a side effect, so the picker offers it next time', async () => {
    const { customCategories, setMerchantCategory } = await import('../src/overrides.ts');
    setMerchantCategory(db, 'Sdm', 'Pharmacy');
    expect(customCategories(db)).toContain('PHARMACY');
    expect(byId('tx:3')?.category).toBe('PHARMACY');
  });

  it('updates the rule that renamed a merchant rather than adding an unmatchable second one', async () => {
    const { listMerchantRules, setMerchantCategory } = await import('../src/overrides.ts');
    addMerchantRule(db, { pattern: 'SDM', merchant: 'Shoppers Drug Mart' });
    // A new rule on "Shoppers Drug Mart" could never match: rules see the
    // institution's raw text, which still says "Sdm SDM 1235".
    setMerchantCategory(db, 'Shoppers Drug Mart', 'PHARMACY');
    expect(listMerchantRules(db)).toHaveLength(1);
    expect(byId('tx:3')?.merchant).toBe('Shoppers Drug Mart');
    expect(byId('tx:3')?.category).toBe('PHARMACY');
  });

  it('uncategorises without losing the rename', async () => {
    const { setMerchantCategory } = await import('../src/overrides.ts');
    setMerchantCategory(db, 'Sdm', 'PHARMACY');
    setMerchantCategory(db, 'Sdm', null);
    expect(byId('tx:3')?.category).toBe('GENERAL_MERCHANDISE');
    expect(byId('tx:3')?.merchant).toBe('Sdm');
  });
});

describe('the merchant directory', () => {
  it('reports the dominant category, not whichever row came first', () => {
    upsertTransactions(db, [
      {
        id: 'tx:5',
        account_id: 'plaid:card',
    ...
    const sdm = groupByMerchant(getTransactions(db, { limit: 50 })).find((g) => g.merchant === 'Sdm');
    // Two MEDICAL against one GENERAL_MERCHANDISE — one stray bank guess must
    // not relabel the merchant.
    expect(sdm?.category).toBe('MEDICAL');
    expect(sdm?.categories).toHaveLength(2);
  });

  it('lists which cards a merchant was paid with', () => {
    const sdm = groupByMerchant(getTransactions(db, { limit: 50 })).find((g) => g.merchant === 'Sdm');
