/**
 * Fixture-based aggregation tests. The figures are invented but deliberately
 * uneven and realistically shaped, so a sign flip or a double count shows up as
 * a number that is obviously wrong rather than subtly off.
 */
...
describe('contribution room', () => {
  beforeEach(() => {
    setRoomLimit(db, 'me', 'RRSP', 2026, 40000, 'example notice of assessment');
    setRoomLimit(db, 'me', 'TFSA', 2026, 25000.5);
    upsertActivities(db, [
      {
        id: 'snaptrade:act:1',
        account_id: 'snaptrade:ws-rrsp',
  ...
  it('subtracts detected contributions for the requested year only', () => {
    ...
    expect(rrsp.remaining_cad).toBe(28000);
  ...
  it('flags low confidence when nothing was detected', () => {
    ...
    expect(tfsa.remaining_cad).toBe(25000.5);
  ...
  it('lets a manual figure override detection', () => {
    ...
    expect(rrsp.remaining_cad).toBe(20000);
    expect(rrsp.contributed_detected_cad).toBe(12000);
  });
